...
from .social_providers import PROVIDERS, SocialAuthError
...
class SocialUserAPI(APIView):
    ...
    Request body:  { "provider": "google" | "facebook", "access_token": "<token>" }

    The access token is verified directly with the provider; the email is read
    from the provider's response, never from the client. Returns the same shape
    as LoginAPI: { message, token, user }.
    """
